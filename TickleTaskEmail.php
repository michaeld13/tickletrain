...
function hasNewCommentsNEWWW($MailID)
    {
    	global $db;
...
$result =mysqli_query($db->conn,"SELECT TickleID,TimeZone,TimeDailyTickle,today_report_date from tickleuser where (TickleID = '575') AND `Status`='Y'");
//and TickleID=575
$users = array();
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    //debug('------For TickleID '.$row['TickleID'].'---------');
     // if ($now  == strtotime($send_date)) {
			echo 'mail sent to this user';
        	$Cdate = gmdate("Y-m-d", $now);
        	$ctime = strtotime(date("Y-m-d", $now) . " 23:59:59");
            
             SendDailyTaskUser_Report($Cdate, $row['TickleID'], $ctime);
...
function SendDailyTaskUser_Report($Cdate, $uid, $ctime) { 
    global $db;
    ...
    $query = "SELECT distinct *, TA.TaskGMDate as task_tm, UM.toaddress as touserrecpint,TU.EmailID as ReportEmailID, CL.EmailID as CEmailID, TU.TimeZone as user_timezone from tickleuser as TU , task as TA, tickle as TT, user_mail as UM,contact_list as CL  WHERE TA.TaskGMDate>='$start_date' and  TA.TaskGMDate<'$end_date'  and TA.Status='Y' and TU.TickleID=TA.TickleID and TA.TickleTrainID = TT.TickleTrainID and TA.TickleID=TT.TickleID and TA.MailID=UM.MailID and UM.ContactID=CL.ContactID and TU.TickleID=$uid  and TT.reminder_task='Y' order by TT.TickleName asc, TA.TaskGMDate asc";
    ...
    echo "Found " . count($tusers) . ' tasks for daily report sent'.'<br/>';
    $contact_list = array();
    if (count($tusers) > 0) {
    ...
    for ($hhh = 0; $hhh < count($tusers); $hhh++) {
       ...
       /* if (trim($row['ccaddress']) != "") {
		...
		if (trim($row['ccaddress']) != "") {
         ...
         //  $ccMail = explode('@', htmlspecialchars($row['ccaddress']));
			$ccMail = explode('@', htmlspecialchars($row['CEmailID']));
			$row['ccaddress'] = $row['CEmailID'];
            if(isset($ccMail[1]) && $ccMail[1]!='tickletrain.com')
            {    
                //$Contact.="<br>CC: " . htmlspecialchars($row['ccaddress']);
				$Contact.="<br>CC: " . htmlspecialchars($row['CEmailID']);
            }    
        }
		...
		$TickleArr[$TickleID][$ddd]['quicktimechange'] = ""; 
		...
		// $subject = "https://" . SERVER_NAME . Url_Create("previewmail", "act=" . rawurlencode($protect) . "&MailID=" . $MailID ."&els=".rawurlencode(protect('yes'))."&Mails=Mail");
       
      $subject = "https://" . SERVER_NAME . Url_Create("previewmail", "act=" . rawurlencode($protect) . "&MailID=" . $MailID . "&Mails=Mail");
        if(isset($row['CustomSubject']))
			$newsub = $row['CustomSubject'];
		else
			$newsub = $row['Subject'];
	   $subject = '<a href="' . $subject . '" style="white-space:nowrap">' . htmlspecialchars($newsub) . '</a>';
		...
		ob_start();
		if(isset($row['ccaddress']) && !empty($row['ccaddress'])):
			include "tpl/task_mailrow.php";
			$htmlrow = ob_get_contents();
			ob_end_clean();
			$TickleMail[$TickleID]['HTML']["normal"][$row['TickleName']][].=$htmlrow;
		else:
			include "tpl/task_mailrow_blank.php";
			$htmlrow = ob_get_contents();
			ob_end_clean();
			$TickleMail[$TickleID]['HTML']["blank"][$row['TickleName']][].=$htmlrow;
		endif;
    ...
    //  }else{
    $txtstart = "Today's Task Reminders for " . $reportDate . ":\n";
    // }
    ...
    if (count($TickleMail) > 0) {
        ...
        foreach ($TickleMail as $Key => $Value) {
			...
			$HTMLValue="<div id='send_taskemail_tdy'>";
			
            if (count($Value) > 0 && $Value['EMAIL'] != "") {
                $EmailPriority = 3;
                ob_start();
                //include "tpl/task_mail.php";
				...
				$keyarry = array();
				//echo '<pre>'; print_r($Value['HTML']); exit;
				/* First Loop */
				if(isset($Value['HTML']['blank'])){
					foreach ($Value['HTML']['blank'] as $subKey => $subValue) {
						$keyarry[] = $subKey; 
						
							$HTMLValue.="<h6 style='font-size:16px;margin:9px !important;' class='ticklenamesendmail' id='ticklenamesendmail'>".ucfirst($subKey)."</h6>";
							$HTMLValue.="<table id='ticklenamesendtable' width='100%' cellspacing='0' cellpadding='0' bgcolor='#ffffff' style='border: 1px solid #e4e4e4;margin-bottom: 2px; table-layout: fixed;'>";
							$HTMLValue.="<thead><tr bgcolor='#e7e7e7' style='line-height:18px;'><th width='37%' align='left' style='padding-left:5px;padding-right:5px;'>Subject</th><th  width='56%' align='left' style='padding-left:5px;padding-right:5px;'>Remind In</th><th  width='15%' align='left' style='padding-left:5px;padding-right:5px;white-space: nowrap;'>Created Date</th>";
							//if($subKey == 'delegate')
								//$HTMLValue.="<th>Recipient</th>";
							
							$HTMLValue.="<th  width='10%' align='left' style='padding-left:5px;padding-right:5px;'>Actions</th></tr></thead>";
							foreach ($subValue as $sskey => $ssVal) {
								$HTMLValue.=$ssVal;
							}	
							$HTMLValue.="</table>";
							$HTMLValue.='<table width="100%" cellspacing="0" cellpadding="0" bgcolor="#ffffff"><tr>
								<td><table width="100%" cellspacing="0" cellpadding="0" bgcolor="#ffffff"></table></td></tr></table>';
				...
				/* First Loop */
				if(isset($Value['HTML']['normal'])){
					
					$HTMLValue.="<h6 class='ticklenamesendmail' style='font-size:20px;margin:9px !important;' id='ticklenamesendmail'>Delegated</h6>";
					foreach ($Value['HTML']['normal'] as $subKey => $subValue) {
						$keyarry[] = $subKey;  
						
							$HTMLValue.="<h6 style='font-size:16px;margin:9px !important;' class='ticklenamesendmail' id='ticklenamesendmail'>".ucfirst($subKey)."</h6>";
							$HTMLValue.="<table id='ticklenamesendtable' width='100%' cellspacing='0' cellpadding='0' bgcolor='#ffffff' style='border: 1px solid #e4e4e4;margin-bottom: 2px; table-layout: fixed;'>";
							$HTMLValue.="<thead><tr bgcolor='#e7e7e7' style='line-height:18px;'><th width='37%' align='left' style='text-overflow: ellipsis; overflow: hidden;padding-left:5px;padding-right:5px;'>Subject</th><th width='56%' align='left' style='text-overflow: ellipsis; overflow: hidden;padding-left:5px;padding-right:5px;'>Remind In</th><th width='12%' align='left' style='padding-left:5px;padding-right:5px;'>Created Date</th>";
							$HTMLValue.="<th width='18%' align='left' style='text-overflow: ellipsis; overflow: hidden;padding-left:5px;padding-right:5px;'>Recipient</th>";
							
							$HTMLValue.="<th width='8%' align='left' style='padding-left:5px;padding-right:5px;'>Actions</th></tr></thead>";
							foreach ($subValue as $sskey => $ssVal) {
								$HTMLValue.=$ssVal;
							}	
							$HTMLValue.="</table>";
							$HTMLValue.='<table width="100%" cellspacing="0" cellpadding="0" bgcolor="#ffffff"><tr>
								<td style="text-overflow:hidden; text-overflow: ellipsis;"><table width="100%" cellspacing="0" cellpadding="0" bgcolor="#ffffff"></table></td></tr></table>';
											
					}
				} 
				
				//$arremail = array('balwinder.shinedezign@gmail.com','anjali.shinedezign@gmail.com');
                $mailreport->SetFrom($TtSmtpReplyMail, "TickleTrain");
                // if(isset($serviceid)){
                //  $mailreport->Subject = "Today's Tickles scheduled for " . $reportDate;
                //  }else{
                $mailreport->Subject = "Today's Task Reminders for " . $reportDate;
                //  }
				...
				//$mailreport->MsgHTML($HTMLValueNEW, IMAGE_BASE_FOLDER);
               // $mailreport->IsHTML(true); // send as HTML
                //$mailreport->AddReplyTo($TtSmtpReplyMail);
                $mailreport->Sender = $TtSmtpReplyMail;
             //   $mailreport->AddAddress('balwinder.shinedezign@gmail.com'); //$Value['EMAIL']
				$mailreport->AddCC('anjali.shinedezign@gmail.com');
                $mailreport->Send();
                echo "Today's Tickles has been sent for '" . $Value['EMAIL'] . "'";
                ...
                if ($Value['ALTEMAIL'] != "") {
                    try {
                            $mailreport->AddReplyTo($TtSmtpReplyMail);
                            $mailreport->Sender = $TtSmtpReplyMail;
                            $mailreport->AddAddress('balwinder.shinedezign@gmail.com');
							$mailreport->AddCC('anjali.shinedezign@gmail.com');
                            $mailreport->Send();
                            // Clear all addresses and attachments for next loop
                            $mailreport->ClearAddresses();
