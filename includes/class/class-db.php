...
class db{
	...
	public function query_to_array($Q_str,$result_type='ASSOC'){
		...
		if(! $this->query($Q_str)){
			return FALSE;
                ...
                $ret = $this->fetch_res($this->last_qres,$result_type);
                if (is_array($ret)){
                    $this->last_q2a_res=$ret;
                }
		return $this->last_q2a_res;
	}
	...
	**/
      public function countcontact(){
          $query = mysql_query("select count(*) as 'c' from contact_list where TickleID = '".$_SESSION['TickleID']."'");
...
$linkStr.="&u=".$_GET['u'];
	...
	* @return int affected_rows or bool (depends on the database implementation (have we a query_affected_rows or not?))
	**/
	public function update_ignore($table,$values,$conds = null){
		if(is_array($values)){
			$str = array();
			foreach( $values as $k=>$v)
				$str[] = $this->protect_field_names($k)." = ".$this->prepare_smart_param($v).' ';
		...
		# now the WHERE str
		$conds_str = $this->process_conds($conds);
		$Q_str = "UPDATE IGNORE $table SET ".(is_array($values)?$this->array_to_str($str):$values)." $conds_str";
		//echo $Q_str;
		if(method_exists($this,'query_affected_rows')){
			$res = $this->query_affected_rows($Q_str);
			return ($res===FALSE || $res === -1)?FALSE:$res;
		...
		elseif(! is_array($conds) )
			return '';
                ...
                if(count($conds) > 1){
                    ...
                    {
                        if($i%2=='0'){        
                            $conds_str .= str_replace("?", $this->prepare_smart_param($conds[$i+1]) , $conds[$i])." and ";
                        }
                    }
                    $conds_str = substr($conds_str, 0,-5);                    
                    return $conds_str;
                }else{
