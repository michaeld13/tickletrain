...
*            - $LastChangedBy: malko $
*            - $HeadURL: http://trac.jgotti.net/svn/class-db/trunk/adapters/class-mysqldb.php $
* @changelog
	...
	function check_conn($action = ''){
		...
		if((! $this->conn) || ! $host = mysql_get_host_info($this->conn)){
			switch ($action){
				...
				default:
				case 'active':
					if(! $this->conn = mysqli ($this->host,$this->user,$this->pass,self::
...
$useNewLink)){
	...
	*/
	...
	/**
	* there's a base method you should replace in the extended class, to use the appropriate escape func 

regarding the database implementation
	...
	* @param bool $extended_info if true will return the result of a show field query in a query_to_array 
...
fashion
	...
	* @param gtkprogress &$progress is an optional progressbar to trace activity (will received a value between 
				...
				foreach($tabledatas as $row){
					...
					foreach($row as $field=>$value){
						if($i==0){ # on the first line we get fields
							$fields[] = "`$field`";
              if( mysql_field_type($this->last_qres,$z++) === 'string') /* will permit to correctly protect number 
