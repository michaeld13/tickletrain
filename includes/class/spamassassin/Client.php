...
require_once 'Client/Result.php';
...
class SpamAssassin_Client
{
    ...
    protected function execshort($cmd, $message, array $additionalHeaders = array())
    {
        $socket        = $this->getSocket();
        $message      .= "\r\n";
        $contentLength = strlen($message);
        ...
        $this->write($socket, $cmd);
        $message = $this->readtrouble($socket);
    ...
    protected function readtrouble($socket)
    {
        $message = array();
        $headers = '';
        $message = '';
        ...
        $trigger=false;
        while (!feof($socket)) {
            $m= trim(fgets($socket, 128));
            if ($trigger) 
                if (is_string($m)) if ($m!='') $message[]= $m;
            if (strncmp($m, '---- ----------------------', 20)==0) $trigger=true;
        }
    ...
    public function getShortSpamReport($message)
    {
        return $this->execshort('REPORT', $message);
    }
