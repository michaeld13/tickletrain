...
if(false)
{
   ...
   echo $query = "SELECT tk.* from task tk inner join user_mail um on um.MailID=tk.MailID WHERE tk.TaskGMDate<='$Cdate' and tk.Status='Y' and tk.TaskID NOT IN (".$logResult.") and tk.TickleID NOT IN (".$userSmtp.") and tk.secondaryEmailId NOT IN (".$secUserSmtp.")  order by tk.TickleID limit 1000";
   ...
   //die;
    ...
    for ($tt = 0; $tt < count($tasks); $tt++) {
        ...
        if (!$err) {
			echo "select * from tickleuser where TickleID='$TickleID' and Status='Y'";
        ...
        if ($rs_user['addon_hosting_id'] != '') {
            ...
            if ($rs_user['email_addon'] != '' && ($status['status'] == 'Active' || $status['status'] == 'Pending')) {
                ...
                if ($row['secondaryEmailId'] != '0') {
                    ...
                    if($secid){
                       echo 'rssuers'.$rs_user['FromEmail'] = $secid['FromEmail'];
       ...
       // echo $toaddress['EmailID'] .' ,'.  $rs_user['EmailID'] ;
        ...
        echo 'replytoo'.$reply_to = $rs_user['EmailID'];
		print_r($rs_user);
       echo 'fromemail'.$FromEmailid = (!empty($rs_user['FromEmail'])) ? $rs_user['FromEmail'] :  $reply_to ;
		...
		echo $CCMe.'==='.$CCMeFollow;
		echo 'sss'.$taskSent;
        if (($CCMe == "Y" || $CCMeFollow == "Y") && $taskSent) {  echo 'ssss';
            ...
            if (!in_array($TaskID, $bccCheck)) {
               ...
               // $mailBcc->SMTPDebug = 2;
				...
				//$mailBcc->AddAddress('anjali.shinedezign@gmail.com');
