...
function showInbox($imap) {
  ...
  for ($i = 1; $i <= $storage->countMessages() && $i <= 5; $i++ ){
    //echo '<li>' . $storage->getMessage($i)->isMultipart() . "</li>\n";
    $storage->getMessage($i)->isMultipart() ;
    $part = $storage->getMessage($i)->getPart(1);

echo 'Type of this part is ' . strtok($storage->getMessage($i)->contentType, ';') . "\n";
echo "Content:\n";
echo $storage->getMessage($i)->getContent()."<hr><br>";
  }
//echo '</ul>';
}
