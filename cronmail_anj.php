...
function update_followups_time($MailID,$TaskID,$FollowTickleTrainID){
    ...
    //{
        ...
        while ($rowT = mysqli_fetch_array($uQuery)) {
           ...
           echo '1111'.$TaskInitiateDateNew = date('Y-m-d H:i:s', mktime(date('H',$rIniDate), date('i',$rIniDate), date('s',$rIniDate), date('m',$rIniDate), date('d',$rIniDate) + 1, date('Y',$rIniDate)));
...
}else
{
    ...
    $query = "SELECT tk.* from task tk inner join user_mail um on um.MailID=tk.MailID WHERE tk.TaskGMDate<='$Cdate' and tk.Status='Y' and tk.TaskID NOT IN (".$logResult.") and tk.TickleID NOT IN (".$userSmtp.") and tk.secondaryEmailId NOT IN (".$secUserSmtp.")  order by tk.TickleID limit 1000";

    //and tk.TickleID='1598'


   //$query = "SELECT tk.* , um.message_id from task tk inner join user_mail um on um.MailID=tk.MailID WHERE tk.Status='Y' and tk.Pause = 'N' and tk.TaskID = 1225686 order by tk.TickleID limit 1000"; 
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if($row['Pause']=='N'){
           ...
           echo '2222'. $TaskInitiateDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskInitiateDate']));
		   echo "update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $row['TaskID'] . "' ";
   ...
   //die;
    ...
    for ($tt = 0; $tt < count($tasks); $tt++) {
        ...
        if ($NoWeekend == 'Y' && ($dofweek == 0 || $dofweek == 6)) {   
            $taskGMDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskGMDate']));
           echo '33333'.$TaskInitiateDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskInitiateDate']));   
echo "update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $TaskID . "' ";		   
        ...
        if ($TApprove == 'Y' && $row['Approve'] != 'Y' && $NoWeekend != 'Y') {
            $taskGMDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskGMDate']));
            echo '44444'.$TaskInitiateDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskInitiateDate']));  
			echo "update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $TaskID . "' ";
            mysqli_query($db->conn,"update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $TaskID . "' ");
            update_followups_time($MailID,$TaskID,$row['FollowTickleTrainID']);
        ...
        if ($row['Pause'] != 'N') {
            $taskGMDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskGMDate']));
           echo '555'.$TaskInitiateDateNew = $tomorrowDate.' '.date('H:i:s', strtotime($row['TaskInitiateDate']));
		   echo "update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $TaskID . "' ";
            mysqli_query($db->conn,"update task set TaskGMDate='" . $taskGMDateNew . "',TaskInitiateDate='" . $TaskInitiateDateNew . "' where TaskID='" . $TaskID . "' ");
            update_followups_time($MailID,$TaskID,$row['FollowTickleTrainID']);
