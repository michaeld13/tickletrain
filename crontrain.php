...
for ($i = $size - 1; $i >= 0; $i--) {
    ...
    $MailFrom = $hd->from[0]->mailbox ."@". $hd->from[0]->host;
    ...
    $mysqls = mysqli_query($db->conn,"select * from tickleuser where UserName='$chaeknamehere' AND EmailID='$MailFrom'");
    $tickle_user = mysqli_fetch_assoc($mysqls);
    ...
    if (!$tickle_user) {
        ...
        if (!$tickle_user) {
            ...
            $mysqls = mysqli_query($db->conn,"select * from secondaryEmail where TickleID='".$tickle_user['TickleID']."' and EmailID='".$MailFrom."'"); 
            $send_from_secondaryEmail = mysqli_fetch_assoc($mysqls);

            if(!$send_from_secondaryEmail){
                goto mailcontinue;
            }
        }
    ...
    $allowed_compaign = $db->select_rows('Compaign', "Allowe_campaign", "where TickleID='$tickle_user[TickleID]'", ASSOC);
    ...
    // print_r($message_body_parsed);
    // die;

    $att_path = array();
    for ($aa = 0; $aa < count($message_body_parsed['attachments']); $aa++) {
       // if($message_body_parsed['attachments'][$aa]['content-disposition'] != 'inline'){
         $image_name_mail = remove_special_characters_from_string($message_body_parsed['attachments'][$aa]['filename']);
		$imageArr = explode('.',$image_name_mail);
		$randimg = $rndnum.$aa;
		$att_path[] = $imageArr[0].'_'.$randimg.'.'.$imageArr[1];
      //}
    }
   //print_r($att_path);
   ...
   // ENd of unset date command code  // 2019-10-29 01:_0 3
    ...
    if (strpos($str,'date_') !== false) { // date_10242019_01_39_PM+quotes+sandeep@tickletrain.com
        ...
        $arr__ =  ['N','1H','2H','3H','1D','2D','3D','1W','2W','1M'];
        ...
        if(!in_array(strtoupper($task_month), $arr__)) {
            ...
            if(in_array($task_month, ['N','1H','2H','3H'])) {
                $task_correct_date_format =  getlocaltime(date('Y-m-d H:i:s',$time__), $tickle_user['TimeZone']);
            }else{
	...
	if (isset($decode_addr['Part3']) && ($decode_addr['Part3'] == 'unpause' || $decode_addr['Part3'] == 'pause')){
		$approve_for_campaign = $decode_addr['Part3'];
		 if($approve_for_campaign == 'unpause')
    ...
    if (strtolower($userEmailId) != strtolower($fromAddr['email']) && strtolower($userEmailId) != strtolower($DefaultFromAddress)) {
        ...
        $secquery = mysqli_query($db->conn,"select * from secondaryEmail where EmailID='".$DefaultFromAddress."' and TickleID='".$tickle_user['TickleID']."'");
	...
	//echo  ("Tickletitle: " . $data_m['TickleTitle']);
	...
	//print_r($globalcmd); 
    if (in_array($tickleTitle, $globalcmd) || preg_match('/^\d+$/', $tickleTitle)) { echo 'herererer';
        ...
        while ($user_mail = mysqli_fetch_array($usmail)) {  echo 'ssss';
    ...
    //global unsubscribe
//echo "select * from tickle where CTickleName='" . strtolower($data_m['TickleTitle']) . "' and TickleID='" . intval($rss['TickleID']) . "' and Status='Y'";
...
//
    ...
    $mytickleid = $tickle_user['TickleID'];
    ...
    if($rst['reminder_task'] ==  'Y' && empty($ccmail_address) ) {

        echo "Reminder To Sender<br>";
        $sender_address = (array) ($MailHeader['sender']);
        foreach ($sender_address as $ind => $val) {

            if (isset($contact_list[$val['email']])) {
                $sender_address[$ind]['contactid'] = $contact_list[$val['email']]['ContactID'];
            } else {
                   $fname = $lname = ""; 
                if ($val['personal'] != '') {
                ...
                mysqli_query($db->conn,"insert into contact_list (EmailID, TickleID, Status,FirstName,LastName) values ('" . $val['email'] . "','" . $data_m['TickleID'] . "','Y','".$fname."','".$lname."')");

                $sender_address[$ind]['contactid'] = mysqli_insert_id($db->conn);
            }
        }
    }
    ...
    if($rst['reminder_task'] ==  'Y' && !empty($ccmail_address) ) {
        ...
        echo "Reminder will added to CC<br>";
    ...
    if($rst['reminder_task'] !=  'Y'){
        ...
        foreach ($tomail_address as $ind => $val) {
            ...
            if (isset($contact_list[$val['email']])) {
                ...
                mysqli_query($db->conn,"insert into contact_list (EmailID, TickleID, Status,FirstName,LastName) values ('" . $val['email'] . "','" . $data_m['TickleID'] . "','Y','".$fname."','".$lname."')") or die(mysqli_error($db->conn) . __LINE__) ;
        ...
        //echo"here now";
		   ...
		   if (!is_dir($Rawmaildir)) {
            ...
            if (count($message_body_parsed['attachments']) != 0) {
                ...
                for ($aa = 0; $aa < count($message_body_parsed['attachments']); $aa++) {
					$newattchimge = remove_special_characters_from_string($message_body_parsed['attachments'][$aa]['filename']);
					$imageArr = explode('.',$newattchimge);
					$randimg = $rndnum.$aa;
					$NewATtachImage = $imageArr[0].'_'.$randimg.'.'.$imageArr[1];
                    file_put_contents($Rawmaildir . $Rmailid . "/" .$NewATtachImage, $message_body_parsed['attachments'][$aa]['content']);
                }
            }
            ...
            if (count($message_body_parsed['images']) != 0) {
              ...
              //  debug("Created '" . $Rawmaildir . $Rmailid . "/" . "'");
                ...
                foreach ($message_body_parsed['images'] as $cid => $fl) {
					$imageArr = explode('.',$fl['filename']);
					$randimg = $rndnum.$aaa;
					$fl['filename'] = $imageArr[0].'_'.$randimg.'.'.$imageArr[1];
                 ...
                 // echo "update user_mail set MessageHtml=replace(MessageHtml,'cid:$cid','" . $image_url . "') where MailID='$Rmailid'";    
				//	exit;
            ...
            mysqli_query($db->conn,"update user_mail set RawPath='$Rawmail', toaddress='".$MailHeader['to'][$ind]['email']."' where MailID='$Rmailid'");
            $logData['toaddress'] = $MailHeader['to'][$ind]['email']; 
               ...
               $task['TimeZone'] = $TimeZone;
                ...
                if ($CReply != "reply") {
                   ...
                   date_default_timezone_set(gettimezone($TimeZone));
					...
					if($rst['DailyDays'] == 0){
						$curnttime = date("Y-m-d H:i:s");
							$curnttime = strtotime($curnttime);
							$curnttime = strtotime("+2 minute", $curnttime);
							$curnttime = date("H:i:s",$curnttime);
						$Task_first['TickleTime'] = $curnttime;  // 12pm for first tickle here
					}
					else{
						$Task_first['TickleTime'] = $rst['TickleTime'];  // 12pm for first tickle here
					}
			...
			elseif (isset($decode_addr['Part3']) && ($decode_addr['Part3'] == 'unpause' || $decode_addr['Part3'] == 'pause')) {
			if ($approve_for_campaign == 'Y') {
                    $pause = 'Y';
                    $approve = 'N';
                } else {
				...
				$update_task_approve_query = mysqli_query($db->conn,"update task set Pause='" . $pause . "',Approve='" . $approve . "' where MailID = '" . $MailID . "' AND FollowTickleTrainID  = 0 ") or die(mysqli_error($db->conn) . __LINE__);
    ...
    if (isset($overlimitemail) && count($overlimitemail) > 0) {
        ...
        $overlimitmailcontent.='<div style="margin:20px auto;text-align:center"><a href="'.$Tdirectupgradelink.'" target="_blank"><button style="background:none repeat scroll 0 0 #008acd;border:1px solid #007fc2;color:#ffffff;font-family:arial;font-size:18px;min-height:30px;width:360px;margin:0;text-decoration:none;cursor: pointer;">CLICK HERE to Upgrade to the Unlimted Plan for less than $5/month</button></a><div></div></div><p style="text-align:center">Having a problem? Please <a href="https://tickletrain.com/contact-us/" target="_blank">contact us.</a></p><br/><br/><p>Thank you for using TickleTrain,</p><p>Send it. And Forget it.</p>';
            $overlimitmailcontent;
            //SendMail($tickle_user['EmailID'], 'noreply@tickletrain.com', 'Overlimit', "$overlimitmailcontent");
        SendMail($tickle_user['EmailID'], 'noreply@tickletrain.com', 'Time to upgrade', "$overlimitmailcontent");
        //debug("Campaigns reach their limit, email to '$tickle_user[EmailID]' has been sent");
        // goto mailcontinue;  
    }
...
function MultiIntervalsFollow($DailyDays, $EndAfter, $TickleTime, &$startday,&$task_correct_date_format,$task_followTickleTrainID, $noweekend = 'N') {
    ...
    while (count($dates) < $EndAfter) {
     ...
     //  $i+=$DailyDays;https://www.youtube.com/watch?v=IdoQCk8P2l8
        ...
        //skip weekend
        $date = date('Y-m-d', $time) . ' ' . $TickleTime;  // 12 pm come herererrerer for instantly
