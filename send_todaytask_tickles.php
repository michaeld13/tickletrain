...
$result =mysqli_query($db->conn,"SELECT TickleID,TimeZone,TimeDailyTickle,today_report_date from tickleuser where (TickleID = '4658825' OR TickleID = '575') AND `Status`='Y'");
//and TickleID=575
$users = array();
    ...
    if (count($TickleMail) > 0) {
        ...
        foreach ($TickleMail as $Key => $Value) {
            ...
            if (count($Value) > 0 && $Value['EMAIL'] != "") {
				...
				if(isset($Value['HTML']['blank'])){
					foreach ($Value['HTML']['blank'] as $subKey => $subValue) {
						$keyarry[] = $subKey; 
							...
							$HTMLValue.="<table id='ticklenamesendtable' width='100%' cellspacing='0' cellpadding='0' bgcolor='#ffffff' style='border: 1px solid #e4e4e4;margin-bottom: 12px; table-layout: fixed;'>";
							$HTMLValue.="<thead><tr bgcolor='#e7e7e7' style='line-height:18px;'><th style='width:250px;'>Subject</th><th style='width:325px;'>Remind In</th><th style='width:100px;'>Created Date</th>";
							//if($subKey == 'delegate')
								//$HTMLValue.="<th>Recipient</th>";
							
							$HTMLValue.="<th style='width:50px;'>Actions</th></tr></thead>";
				...
				/* Second Loop */
				//if(in_array('delegate',$keyarry) || in_array('errands',$keyarry)){
				//if($subKey == 'delegate' || $subKey == 'errands'){
				...
				if(isset($Value['HTML']['normal'])){
					...
					foreach ($Value['HTML']['normal'] as $subKey => $subValue) {
						$keyarry[] = $subKey; 
							...
							$HTMLValue.="<h6 style='font-size:16px;margin:9px !important;' class='ticklenamesendmail' id='ticklenamesendmail'>".ucfirst($subKey)."</h6>";
							$HTMLValue.="<table id='ticklenamesendtable' width='100%' cellspacing='0' cellpadding='0' bgcolor='#ffffff' style='border: 1px solid #e4e4e4;margin-bottom: 12px; table-layout: fixed;'>";
							$HTMLValue.="<thead><tr bgcolor='#e7e7e7' style='line-height:18px;'><th style='width:250px; text-overflow: ellipsis; overflow: hidden;'>Subject</th><th style='width:325px; text-overflow: ellipsis; overflow: hidden;'>Remind In</th><th style='width:100px;'>Created Date</th>";
							$HTMLValue.="<th style='width:200px; text-overflow: ellipsis; overflow: hidden;'>Recipient</th>";
				...
				$HTMLValue.='<font face="Arial, Helvetica, sans-serif" size="2" color="#5d5d5d" style="font-size: 14px;">'.$Value['view_more'].'<a href='.$Value['TDashboardLink'].' style="text-decoration: none;"><font color="#0088c2"> Click here</font></a> to login to TickleTrain</font>';
				$HTMLValue.= "</div>";
